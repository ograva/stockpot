...
if (TEST_ENV === 'production') {
  throw new Error(
    ...
    'Example: TEST_ENV=local npm run test:e2e:user'
  );
}

const BASE_URLS: Record<string, string> = {
  local: 'http://localhost:4400',
  staging: 'https://user-staging.flexy-app.com', // Update with actual staging URL
};
...
console.log(`🎭 Playwright User App Tests - Environment: ${TEST_ENV}, Base URL: ${baseURL}`);

/**
 * Playwright configuration for User App E2E tests
...
export default defineConfig({
  ...
  reporter: [
    ['list'],
    ['html', { outputFolder: 'test-results/user-app/html', open: 'never' }],
    ['json', { outputFile: 'test-results/user-app/results.json' }],
    ['junit', { outputFile: 'test-results/user-app/junit.xml' }]
  ],
  ...
  projects: [
    ...
    // Mobile viewports (higher priority for user app)
    {
      name: 'Mobile Chrome',
      use: { ...devices['Pixel 5'] },
    },
